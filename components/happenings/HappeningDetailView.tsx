...
interface HappeningDetailViewProps {
...
export function HappeningDetailView({
  locale,
  item,
  backLabel,
  bookLabel,
  relatedLabel,
  highlightsLabel,
}: HappeningDetailViewProps): React.ReactElement {
  ...
  const meta = [item.location, formatDate(item.eventDate, locale)].filter(Boolean);
  const body = item.body?.trim();

  return (
    <article className={pageBottom}>
      <section className="relative overflow-hidden border-b border-white/5">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_70%_50%_at_80%_-10%,rgba(34,211,238,0.16),transparent),radial-gradient(ellipse_40%_40%_at_10%_90%,rgba(14,165,233,0.1),transparent)]" />
        <div className={`${siteContainer} relative py-8 sm:py-10 lg:py-12`}>
          <ScrollReveal>
            <Link
          ...
          <ScrollReveal className="mt-6">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-400/90">
              {locale === "ar" ? "فعالية مباشرة" : "Live event"}
            </p>
            <h1 className={`mt-3 max-w-4xl ${pageTitle}`}>{item.title}</h1>
            {meta.length > 0 ? (
          ...
          <div className="mt-10 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-12">
            ...
            <ScrollReveal>
              ...
              <EventDetailBook
                label={bookLabel}
                source={`happening-detail:${item.slug}`}
                title={item.title}
              />
            </ScrollReveal>
          </div>
